...
ZipVoice Vietnamese TTS — local Gradio GUI.

Offline-ready after running setup once.
...
from config import ASSETS_DIR, OUTPUT_DIR, ROOT, ensure_ffmpeg_on_path, is_force_cpu, models_ready
...
if not models_ready():
    ...
    print(
        ...
        "\n[ERROR] Models not found. Run setup first:\n"
...
from infer_engine import ZipVoiceEngine  # noqa: E402
...
def _refresh_voices() -> tuple[gr.Dropdown, str]:
    ...
    if not _voice_cache:
        ...
        info += (
            ...
            "và đường dẫn `audio_path` (vd: `data/ref_audio/ten.wav`)."
...
def _on_voice_pick(voice_id: str) -> tuple[str | None, str, str]:
    ...
    if voice is None:
        ...
        return None, "", f"Chế độ: upload thủ công"
...
def infer_tts(
    ...
    try:
        ...
        logger.info(
            ...
            "infer_tts | asset=%s | gen_len=%d | ref_len=%d | export=%s | norm=%s | chunk=%d",
        ...
        for i, tts_chunk in enumerate(tts_chunks):
            ...
            progress(
                ...
                desc=f"Đang tổng hợp đoạn {i + 1}/{len(tts_chunks)}...",
            ...
            if not normalized_preview:
                ...
                normalized_preview = normalized[:500] + ("…" if len(normalized) > 500 else "")
        ...
        status = (
            ...
            f"**Chuẩn hóa:** {norm_label} · **Chunks:** {len(tts_chunks)} "

            f"(max {chunk_max_chars} ký tự/chunk, ~{est_min} phút CPU ước tính)\n\n"
        ...
        # Gradio Audio accepts filepath for mp3/wav playback + download
...
# ZipVoice Vietnamese TTS (2500h)



Giọng mẫu từ thư mục `assets/` · File xuất lưu vào `output/`
...
- `assets/ref_info.json` — danh sách giọng + transcript (`text`)

- `data/ref_audio/*.wav` — file audio (đường dẫn trong JSON)

- `output/` — file xuất `.wav` / `.mp3` (32k / 128k, 24kHz)
...
if __name__ == "__main__":
    ...
    logger.info("Starting Gradio on http://%s:%s (cpu=%s)", host, port, is_force_cpu())
