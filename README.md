...
- **Pipeline chuẩn hóa** tối đa 3 bước: VieNeu / Cấu trúc TTS / vinorm / vietnormalizer / sea-g2p
...
| **`utils.py`** | Pipeline normalize 3 bước, `split_text_for_tts`, preview text, `read_text_file` |
| **`vieneu_text.py`** | Dọn punctuation (port VieNeu-TTS) |
...
4. Chọn **pipeline chuẩn hóa** (Bước 1–3) và **max ký tự/chunk**; có thể **Xem trước chuẩn hóa**
...
| Backend | pip? | Vai trò |
|---------|------|---------|
| **VieNeu** | Không | Dọn punctuation/noise (VieNeu-TTS) |
| **Cấu trúc TTS** | Không | `mẫu (mẹ)`→`mẫu, mẹ`; `một. đoạn`→xuống dòng + nghỉ ~1s |
| **vinorm** | Có | NSW — **không có trong ZipVoice**, PyPI riêng |
| **vietnormalizer** | Có | Chuẩn hóa tiếng Việt rộng |
| **sea-g2p** | Có | NSW; **chỉ Normalizer**, không G2P |
| **Không** | — | Bỏ qua bước |

**Mặc định GUI:** Bước 1 = VieNeu, Bước 2 = Cấu trúc TTS.

**Gợi ý sách:** `VieNeu → Cấu trúc TTS → vinorm` (hoặc sea-g2p).
